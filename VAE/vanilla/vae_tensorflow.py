...
(LOSS, crossentrophy, KL) iter0: (750,738,14); iter5000: (118,94,25); iter10000: (105,86,26).
...
for it in range(10000):
    ...
    _, loss,CE,KL = sess.run([solver, vae_loss,recon_loss,kl_loss], feed_dict={X: X_mb})

    if it % 1000 == 0:
        print('Iter: {}'.format(it))
        print('Loss: {:.4}; CrossEn: {:.4}; KL: {:.4}'.format(loss,np.mean(CE),np.mean(KL)))
        print()
